...
configuration_ytk <- configuration |>
  dplyr::filter(species == "darkblotched rockfish")

configuration_ytk_wcgbt <- configuration_ytk |>
  dplyr::filter(source == "NWFSC.Combo" & family == "sdmTMB::delta_lognormal()")
...
save_index_outputs(
  ...
  indices = index,
  dir_main = paste0(getwd(), "/darkblotched_cog")
)

write.csv(index$cogs,"cog_darkblotched.csv")
